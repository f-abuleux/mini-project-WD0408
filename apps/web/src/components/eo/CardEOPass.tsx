...
export default function CardEOPass() {
    const [data, setData] = useState<Event[]>([]);
    
    useEffect(() => {
        const getEvent = async () => {
            const token = await Cookies.get("token")
            const event = await axios.get("http://localhost:8000/api/event/events/pass", {
                method: "GET",
                headers: {
    ...
    return (
        <div >
                <div className="flex justify-center font-bold text-secondary text-2xl">EVENT ENDED</div>
